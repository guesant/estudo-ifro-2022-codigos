...
UPDATE Cliente SET status_cli = 'Vip' WHERE (renda_familiar_cli > 3000);
# 2. Altere o status para ‘Médio’ nos registros dos clientes com renda familiar entre R$ 1.000,00 e R$3.000,00.
UPDATE Cliente SET status_cli = 'Médio' WHERE (renda_familiar_cli BETWEEN 1000 AND 3000);
# 3. Altere o status para ‘Simples’ nos registros dos clientes com renda familiar menor que R$ 1.000,00.
UPDATE Cliente SET status_cli = 'Simples' WHERE (renda_familiar_cli < 1000);
# 4. Dê um aumento de salário de 30% para os funcionários do sexo ‘Feminino’.
UPDATE Funcionario SET salario_func = salario_func * 1.3 WHERE (id_sex_fk = 2);
# 5. Dê um aumento de R$ 50,00 no salário dos funcionários com função de ‘Vendedor’.
UPDATE Funcionaro SET salario_func = salario_func + 50 WHERE (funcao_func = 'Vendedor');
# 6. Reduza em 10% no salário dos funcionários com função de ‘Mecânico’ e sexo ‘Masculino’.
UPDATE Funcionaro SET salario_func = salario_func * 0.9 WHERE (funcao_func = 'Mecânico') and (id_sex_fk = 1);
# 7. Reduza o valor em 30% nos produtos com estoque maior que 20.
UPDATE Produto SET valor_prod = valor_prod * 0.7 WHERE (quant_prod > 20);
# 8. Dê um desconto de 15% no valor total das vendas com forma de pagamento ‘A vista’ e com valor maior do que R$ 1.000,00.
UPDATE Venda SET valor_total_vend = valor_total_vend * 0.85 WHERE (forma_recebimento_vend = 'A vista') AND (valor_total_vend > 1000);

# Prática com SELECT:
ALTER TABLE Cliente ADD email_cli VARCHAR(400);
...
SELECT * FROM Cliente WHERE (data_nascimento_cli >= '1990-01-01') ORDER BY data_nascimento_cli DESC;
...
SELECT * FROM Funcionario WHERE (salario_func BETWEEN 500 AND 4000) and (id_sex_fk = 2) ORDER BY nome_func;
...
SELECT max(renda_familiar_cli) FROM Cliente;
...
DELETE FROM Pagamentos WHERE (id_comp_fk IS NOT NULL) AND (valor_pag < 800) AND (data_pagamento_pag BETWEEN '2017-01-01' AND '2017-06-30');