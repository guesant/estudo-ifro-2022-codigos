...
# - Adicione seu nome na primeira linha na forma de comentário.
# - Faça os comandos SQL no local específico indicado para cada exercício dentro do script;
...
# 1. Altere o nome do banco nº 2 para 'Bradesco'.
UPDATE Banco SET nome_ban = 'Bradesco' WHERE (cod_ban = 2);
# 2. Altere o nome da agencia nº 1824 para 'Filial'
UPDATE Agencia SET nome_ag = 'Filial' WHERE (numero_ag = '1824');
# 3. Aumente em 30% o salario dos clientes do sexo feminino e nascidos após os anos 2000.
UPDATE Cliente SET renda_cli = renda_cli * 1.3 WHERE (sexo_cli = 'F') AND (data_nascimento_cli >= '2000-01-01');
# 4. Diminua em 5% o salários dos clientes do sexo masculino e nascidos entre os anos 1990 e 2000 e com renda entre 1000 e 5000.
UPDATE Cliente SET renda_cli = renda_cli * 0.95 WHERE (sexo_cli = 'M') AND ((data_nascimento_cli >= '1990-01-01') AND (data_nascimento_cli <= '2000-12-31')) AND ((renda_cli >= 1000) AND (renda_cli <= 5000));
# 5. Aumente em 10,00 reais a taxa mensal dos cliente com valor do limite maior que 1000 reais e saldo inferir a 2000 reais e com data de abertura superior a 01 de janeiro de 2009.
UPDATE Conta_Corrente SET taxa_mensal_cc = taxa_mensal_cc + 10 WHERE (valor_limite_cc > 1000) AND (saldo_cc < 2000) AND (data_abertura_cc > '2009-01-01');
# 6. Aumente em 15% o valor dos saques realizados no 'Caixa Eletrônico' e com valor superior a 50 reais.
UPDATE Saque SET valor_saq = valor_saq * 1.15 WHERE (local_saq = 'Caixa Eletrônico') AND (valor_saq > 50);
# 7. Altera a descrição das transferências para 'Não mencionada' onde a descrição está null ou vazio.
UPDATE Transferencia SET descricao_trans = 'Não mencionada' WHERE (descricao_trans = '' OR descricao_trans = NULL);
# 8. Altere o tipo dos depósitos com valor inferir a 500 ou superior a 4000 para 'Dinheiro'.
UPDATE Deposito SET tipo_dep = 'Dinheiro' WHERE (valor_dep < 500) OR (valor_dep > 4000);
# 9. Aumente em 10% o valor dos pagamentos realizados após junho de 2015 e do tipo 'Boleto' ou 'Convênio'.
UPDATE Pagamento SET valor_pag = valor_pag * 1.1 WHERE (data_pag > '2015-06-30') AND ((tipo_pag = 'Boleto') OR (tipo_pag='Convênio'));
# 10. Altere os dados da agência nº 2 para número 457 e nome 'Centro' .
UPDATE Agencia SET nome_ag = 'Centro', numero_ag = '457' WHERE (cod_ag = 2);
